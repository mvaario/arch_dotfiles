...
for theme in "$COLOR_DIR"/*.sh; do
    ...
    # create wallpaper if not found
    if [ ! -f "$wall" ]; then
        echo "Resizing image" $name
        wall=$(~/.config/themes/scripts/resize_wallpaper.sh "$name")
    fi
    list+=$'img:'"$wall"$'\n'
done
...
nohup "$HOME/.config/themes/scripts/apply_theme.sh" "$theme_file" >/dev/null 2>&1 &
